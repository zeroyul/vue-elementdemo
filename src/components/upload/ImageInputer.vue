  ...
  <div class="img-uploader"
    ...
    <div v-if="hasImages" class="img-uploader-preview-list">
      <div v-for="(data,index) in imageDataList" class="img-uploader-preview">
        ...
        <i class="iconfont icon-icon-guanbi img-uploader-delete-btn" @click="deleteImg(index)"></i>
    ...
    <input
      ...
      ref="input"
      type="file"
      accept="image/gif,image/jpeg,image/jpg,image/png,image"
      multiple="multiple"
...
<script>
  import resizeImage from '@/components/upload/resize';
  export default {
    props: {
      // 占位文字
      placeholder: {
...
//        this.onChange && this.onChange(files)
        this.$emit('onChange', files);//子组件可使用$emit触发父组件的自定义事件
        this.preview(files);
      },
      // 预览图片
      preview (files) {
